  ...
  _createClass(GridItem, [{
    ...
    value: function onDragHandler(handlerName) {
      ...
      return function (e, _ref3) {
        ...
        var callHandlerProp = function callHandlerProp(handlerName) {
        ...
        var start = function start() {
          _this2.setState({ dragStarted: true, timeout: null });
        ...
        switch (handlerName) {
          case 'onDragStart':
            ...
            if (_this2.props.dragDelay === 0) {
              ...
              var _timeout2 = setTimeout(start, _this2.props.dragDelay);
              _this2.setState({ timeout: _timeout2 });
            }
          ...
          case 'onDrag':
            ...
            var _totalDelta = _this2.state.totalDelta || { x: 0, y: 0 };
            _totalDelta.x += deltaX;
            _totalDelta.y += deltaY;
            _this2.setState({ totalDelta: _totalDelta });

            if (!_this2.state.dragStarted) {
              if (Math.max(Math.abs(_totalDelta.x), Math.abs(_totalDelta.y)) > 10) {
...
GridItem.propTypes = {
  ...
  dragDelay: _react.PropTypes.number
};
